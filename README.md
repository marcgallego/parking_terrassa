...
| `/api/status` | estat de la captura: `healthy`, `issues`, minuts des de l'última lectura, lectures d'avui, errors recents i canvis de capacitat. Respon **503** si la captura no va bé. Amb `?totals=1` hi afegeix el total de files, que obliga a recórrer tota la taula |
...
npm run dev:prod                                     # pàgines locals amb dades de producció (només lectura) a http://localhost:8788
...
npm run build                                        # només compila web/ -> public/build/
...
Els filtres queden a l'URL, de manera que una vista es pot compartir: `/` admet `?srdays=30|90|365` i `?threshold=10|25|50`; `/saba` admet `?range=today|7|30` i `?weeks=4|8|26`. Totes dues admeten `?theme=light|dark`. `/sant-roc` redirigeix a `/`, i els enllaços antics a `/` amb `?range` o `?weeks` porten a `/saba`.
